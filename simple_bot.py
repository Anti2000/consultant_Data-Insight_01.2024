...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.from_user.id not in context.bot_data.keys():
        context.bot_data[update.message.from_user.id] = 10
...
async def data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # dial = dial['history']
    # print('dial from outside: ', (dial))
   

    # создаем json и сохраняем в него словарь context.bot_data
    with open('14_homework/data.json', 'w', encoding='utf-8') as fp:
        # dict1 = {}
        # dict1['dials'] = dial
        # print(dict1)
        #dials = {'saving of dials: ', dial}
       # print(dials)
        #dial = str(dial)
        # print('check type: ', context.bot_data)
        # data = {context.bot_data, {'dial_history':dial}}
        dial = context.chat_data['history']
...
async def text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data[update.message.from_user.id] > 0:
        ...
        dial = context.chat_data
        dial = dial['history']
        # print('dial from outside: ', (dial))
        chat = update.message.text + ':' + res['message']
        dial.append(chat)
        # print('adding of chat in dial:', dial)
        context.chat_data[update.message]= dial
        # print(dial)
        if len(dial) >= 6:
            dial.pop(0)
...
def main():
    ...
    #job_queue.run_daily(callback_daily, (time=19,00,00))
    
    # dial = context.chat_data
    # dial = dial['history']
    # print('dial from outside: ', (dial))
    job_queue.run_repeating(callback_daily, interval=6000, first=3)       # первый запуск функции (через сколько секунд)
